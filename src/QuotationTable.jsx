...
function QuotationTable({ data, clearDataItems, deleteByIndex }) {
  ...
  const total = data.reduce((acc, v) => acc + v.qty * v.ppu - v.discount, 0); // Adjust total calculation
  const totalDiscount = data.reduce((acc, v) => acc + parseFloat(v.discount || 0), 0); // Calculate total discount
      ...
      <Table striped bordered hover>
        <thead>
          <tr>
            ...
            <th className={style.textCenter}>Discount</th> {/* Add Discount Column */}
            <th className={style.textCenter}>Amount</th>
          </tr>
        </thead>
        <tbody>
          {data.map((v, i) => {
            let amount = v.qty * v.ppu - v.discount; // Adjust amount calculation
            return (
              <tr key={i}>
                ...
                <td className={style.textCenter}>{v.discount}</td> {/* Add Discount Cell */}
        ...
        <tfoot>
          <tr>
            ...
            <td className={style.textCenter}>{totalDiscount}</td> {/* Add Total Discount */}
            <td></td>
          </tr>
          <tr>
            <td colSpan={4} className={style.textRight}>
            ...
            <td colSpan={2} className={style.textRight}>{total}</td>
          </tr>
        </tfoot>
      </Table>
    </Container>
